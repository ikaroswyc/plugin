...
public class ReplaceClassNameAnalyzer extends ProjectAnalyzer {
   ...
   void replaceClassName(ICompilationUnit[] iCompilationUnits) throws JavaModelException, MalformedTreeException, BadLocationException {
      ...
      {
    	  String curICUName = iCunit.getElementName().replace(".java", "");
    	  if(curICUName.equals(this.newClassName)) 
    	  {
    		  MsgUtil.openWarning("The name " + curICUName + " is already exit. Please pick a different name.");
    		  return;
    	  }
      }
